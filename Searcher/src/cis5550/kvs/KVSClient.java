...
public class KVSClient implements KVS {
  ...
  private static final Logger logger
          = Logger.getLogger(KVSClient.class);
  String coordinator;
  ...
  public synchronized void downloadWorkers() throws IOException {
...
//    System.out.println("kvs coordinator: " + coordinatorArg);
  ...
  public Row getRow(String tableName, String row) throws IOException {
    if (!haveWorkers)
      downloadWorkers();
    System.out.println("KVSClient getRow:");
    System.out.println("rowkey: " + row );
    System.out.println("URL encoded rowkey: " + URLEncoder.encode(row, "UTF-8") );
...
/*  public void persist(String tableName) throws IOException {
  ...
  public String clientTable() throws IOException {
    // Ensure we have the latest list of workers
    if (!haveWorkers) {
      downloadWorkers();
    }
    ...
    // Iterate through the workers and append rows to the table
    for (WorkerEntry worker : workers) {
      sb.append("<tr>")
              .append("<td>").append(worker.id).append("</td>")
              .append("<td>").append(worker.address).append("</td>")
              .append("</tr>");
    }
