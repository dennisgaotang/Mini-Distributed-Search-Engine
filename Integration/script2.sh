...
javac -d classes --source-path src src/cis5550/jobs/Crawler.java

# Compile Indexer class
javac -d classes --source-path src src/cis5550/jobs/Indexer.java

# Compile PageRank class
javac -d classes --source-path src src/cis5550/jobs/PageRank.java
...
# Compile all other Java sources, excluding Searcher.java
javac --source-path src -d bin $(find src -type f -name '*.java' ! -path 'src/cis5550/jobs/Searcher.java')
...
do
    ...
    mkdir -p $dir/pt-pageranks  # Create pt-url folder under each worker directory
