...
class Coordinator extends cis5550.generic.Coordinator {
  ...
  public static KVSClient kvs;
    ...
    post("/submit", (request,response) -> {
      ...
      try {
    	FlameContextImpl flameContextInstance = new FlameContextImpl(jarName, kvs.getCoordinator());
        Loader.invokeRunMethod(jarFile, className, flameContextInstance, argVector);
        // If the method returns normally, the POST request should return a 200 status and, 
        // in the body, any outputs that the method passed to FlameContext.output(). 
        // (If output() was invoked more than once, its argument should be concatenated.)
        //
        response.status(200, "OK");
        String finalOutput = flameContextInstance.getFinalOutput();
