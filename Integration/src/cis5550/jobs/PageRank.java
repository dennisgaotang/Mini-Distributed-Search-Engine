...
public class PageRank {
	public static void run(FlameContext ctx, String[] args) throws Exception {
		try {
			...
			// read the crawl results from pt-url into RDD
			FlameRDD  rdd = ctx.fromTable("pt-url", (row) -> {
				// filter out rows without page columns
				if (!(row.columns().contains("url_childs") && row.columns().contains("url"))) {
					return null;
				}
				String rowKey = row.key();
				String url = row.get("url");
				if (!rowKey.equals(Hasher.hash(url))) {
					System.out.println("!**! error unmatch hash in pt-url");
				}
			...
			System.out.println("filtered pt-url rdd size = " + rdd.count());
			...
			FlamePairRDD stateTable = rdd.mapToPair((str) -> {
				...
				String url_childs = parts[2];

				return new FlamePair(hashedUrl, "1.0,1.0," + url_childs);
			});
...
//					System.out.println(currentNodeHashedURL + "," + rC + "," + rP);
//					System.out.println("parts.length = " + parts.length);
//					if (parts.length > 2) System.out.println("part2 child urls:" + parts[2]);
...
//						System.out.println("n == 0, no outgoing nodes...");
						...
						for (int i = 2; i < parts.length; i++) {
							String pointToUrlHashed = parts[i];
//							System.out.println("point to urls: " + pointToUrlHashed);
							double v = 0.85 * rC / n;
			...
			stateTable.flatMapToPair((p) -> {
				...
				try {
					...
					System.out.println("!**! Row in hashedURL in processed stateTable that's not in pt-url: rowKey= " + hashedURL);
				}
				String state = p._2();
