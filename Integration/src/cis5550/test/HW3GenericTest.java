...
System.out.println("\n"+explanation);
      ...
      try {
        while (buffer.size()<MAXIMUM_DUMP_SIZE) {
          ...
          if ((System.currentTimeMillis()-startTimeMs)>2000) {
            testFailed("Reading the extra data from the server took more than 2 seconds; aborting this cis5550.test.");
            return;
          }
          if (b == -1)
            break;
  ...
  Response readAndCheckResponse(Socket s, String which, boolean expectContentLength, boolean isHEAD) throws Exception  {
    ...
    while (matchPtr < 4) {
      int b = 0;
      if (isTimeout) {
        testFailed("This cis5550.test timed out after "+timeoutMillis+"ms");
    ...
    if (!isHEAD) {
      ...
      for (int i=0; (cl == null) || (i<Integer.valueOf(cl).intValue()); ) {
        ...
        try {
          ...
          if (expectContentLength)
            testFailed("A timeout occurred before the entire message body could be read. We got "+i+" bytes, but expected "+cl+". "
                      ...
                      +", and, if so, whether the value in that header is correct. An off-by-one error can easily cause this cis5550.test to fail. ");
          else
        ...
        if (nRead<0) {
          if (cl != null) {
            testFailed("The server closed the connection before the entire message body could be read. Here is what we received so far:\n\n"+dump(buffer.toByteArray())+"\nCheck whether you sent a Content-Length header (the value we saw was: '"+cl+"'), and, if so, whether the value in that header is correct. An off-by-one error can easily cause this cis5550.test to fail.");
            return null;
          } else {
            break;
          }
